...
class Solution {
     ...
     *   - We can treat it as "infinity" when comparing
      minimums.
     */
        int[] dp = new int[amount + 1];

        // Fill dp[] with a large value representing 
        //impossible (infinity)
        Arrays.fill(dp, amount + 1);

        // Base case:
        // Minimum number of coins needed to form 
        //amount 0 is 0
        dp[0] = 0;

    /**
     * Build the solution from amount 1 up to the
      final 'amount'.
     * For each amount i, try every coin.
     * If the coin value is <= i, then:
     *   we check dp[i - coin] (the best solution
      to form the remaining amount).
     *   dp[i] becomes the minimum among all possibilities.
     */
    ...
    // Only valid if coin does not exceed
    // current amount
                if (i - coin >= 0) {

    /**
     * dp[i - coin] is the minimum coins
      needed to form the remaining amount.
